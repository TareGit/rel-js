...
import { IDatabaseGuildSettings, IDatabaseUserSettings, IGuildSettings, IUserSettings } from "../types";
...
async function updateGuilds() {
    ...
    try {
        const rows = (await bus.db.get(
            `/guilds?q=${bus.guildsPendingUpdate.join(",")}`
...
async function updateUsers() {
    ...
    try {
        const rows: IDatabaseUserSettings[] = (await bus.db.get(`/users?q=${bus.usersPendingUpdate.join(',')}`)).data;

        rows.forEach(function (dbSetting, index) {

            bus.userSettings.set(dbSetting.id, { ...dbSetting, options: new URLSearchParams(dbSetting.options) });

            bus.usersPendingUpdate.splice(bus.usersPendingUpdate.indexOf(dbSetting.id), 1);
...
export async function onJoinedNewGuild(guild) {
    const guildId = guild.id;

    const settings = getDefaultGuildSettings(guildId);

    bus.guildSettings.set(guildId, settings);
}
...
export async function load() {
    ...
    axios
        ...
        .catch((error) =>
            utils.log("Error asking server to track guild updates : ", error.message)
        );

    try {
        const rows: IDatabaseGuildSettings[] = (await bus.db.get(
            `/guilds?q=${bus.guildsPendingUpdate.join(",")}`
        )).data;
    ...
    const settingsToPush: IDatabaseGuildSettings[] = bus.guildsPendingUpdate.map(function (guildId) {
        const setting = getDefaultGuildSettings(guildId);

        bus.guildSettings.set(guildId, convertFromDbSetting(constants.DEFAULT_GUILD_SETTINGS));

        return { ...constants.DEFAULT_GUILD_SETTINGS }
    });



    await bus.db
        .put(`/guilds`, settingsToPush)
        .catch(logPossibleAxiosError);
