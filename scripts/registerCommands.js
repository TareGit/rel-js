...
const botId = isAlpha
  ? process.env.DISCORD_BOT_ID_ALPHA
  : process.env.DISCORD_BOT_ID;

const url = sendGlobalCommands
  ? `https://discord.com/api/v8/applications/${botId}/commands`
  : `https://discord.com/api/v8/applications/${botId}/guilds/919021496914018346/commands`;
...
axios
  .put(url, deleteCommands ? [] : commands, { headers: headers })
  .then((response) => {
    utils.log(
      `Successfully ${deleteCommands ? "Deleted" : "Updated"} commands`
    );
  })
  .catch((error) => {
